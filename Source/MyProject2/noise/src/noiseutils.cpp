...
namespace noise
{
  ...
  namespace utils
  {
    ...
    inline noise::noise_uint8 BlendChannel (const noise_uint8 channel0,
      const noise_uint8 channel1, float alpha)
    {
      float c0 = (float)channel0 / 255.0;
      float c1 = (float)channel1 / 255.0;
      return (noise::noise_uint8)(((c1 * alpha) + (c0 * (1.0f - alpha))) * 255.0f);
    }
    ...
    // hope.)
    inline noise::noise_uint8* UnpackFloat (noise::noise_uint8* bytes, float value)
    {
      noise::noise_uint8* pBytes = (noise::noise_uint8*)(&value);
      bytes[0] = *pBytes++;
      bytes[1] = *pBytes++;
      bytes[2] = *pBytes++;
      bytes[3] = *pBytes++;
    ...
    inline noise::noise_uint8* UnpackLittle16 (noise::noise_uint8* bytes,
      noise::noise_uint16 integer)
    {
      bytes[0] = (noise::noise_uint8)((integer & 0x00ff)      );
      bytes[1] = (noise::noise_uint8)((integer & 0xff00) >> 8 );
      return bytes;
    }

    // Unpacks a 32-bit integer value into four bytes in little endian format.
    inline noise::noise_uint8* UnpackLittle32 (noise::noise_uint8* bytes,
      noise::noise_uint32 integer)
    {
      bytes[0] = (noise::noise_uint8)((integer & 0x000000ff)      );
      bytes[1] = (noise::noise_uint8)((integer & 0x0000ff00) >> 8 );
      bytes[2] = (noise::noise_uint8)((integer & 0x00ff0000) >> 16);
      bytes[3] = (noise::noise_uint8)((integer & 0xff000000) >> 24);
      return bytes;
    }
...
void WriterBMP::WriteDestFile ()
{
  ...
  noise::noise_uint8* pLineBuffer = NULL;
  ...
  try {
    pLineBuffer = new noise::noise_uint8[bufferSize];
  }
  catch (...) {
    throw noise::ExceptionOutOfMemory ();
  }
  ...
  // Build the header.
  noise::noise_uint8 d[4];
  os.write ("BM", 2);
  os.write ((char*)UnpackLittle32 (d, destSize + BMP_HEADER_SIZE), 4);
  os.write ("\0\0\0\0", 4);
  os.write ((char*)UnpackLittle32 (d, (noise::noise_uint32)BMP_HEADER_SIZE), 4);
  os.write ((char*)UnpackLittle32 (d, 40), 4);   // Palette offset
  os.write ((char*)UnpackLittle32 (d, (noise::noise_uint32)width ), 4);
  os.write ((char*)UnpackLittle32 (d, (noise::noise_uint32)height), 4);
  ...
  os.write ((char*)UnpackLittle32 (d, (noise::noise_uint32)destSize), 4);
  ...
  for (int y = 0; y < height; y++) {
    ...
    noise::noise_uint8* pDest   = pLineBuffer;
  ...
  const Color& backgroundColor, double lightValue) const
{
  ...
  // Rescale the color channels to the noise::noise_uint8 (0..255) range and return
  // the new color.
  Color newColor (
    (noise::noise_uint8)((noise::noise_uint)(red   * 255.0) & 0xff),
    (noise::noise_uint8)((noise::noise_uint)(green * 255.0) & 0xff),
    (noise::noise_uint8)((noise::noise_uint)(blue  * 255.0) & 0xff),
  ...
  double bumpHeight) const
{
  ...
  // range.
  noise::noise_uint8 xc, yc, zc;
  xc = (noise::noise_uint8)((noise::noise_uint)((floor)((vxc + 1.0) * 127.5)) & 0xff);
  yc = (noise::noise_uint8)((noise::noise_uint)((floor)((vyc + 1.0) * 127.5)) & 0xff);
  zc = (noise::noise_uint8)((noise::noise_uint)((floor)((vzc + 1.0) * 127.5)) & 0xff);
